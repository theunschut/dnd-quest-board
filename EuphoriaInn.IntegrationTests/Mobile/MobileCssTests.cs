...
public class MobileCssTests : IClassFixture<WebApplicationFactoryBase>
{
    ...
    /// until a directory named "EuphoriaInn.Service" is found (the repo root is its parent).
    ...
    private static string ResolveMobileCssPath()
    {
        ...
        // the presence of an "EuphoriaInn.Service" child directory.
        while (dir != null)
        {
            var candidate = Path.Combine(dir.FullName, "EuphoriaInn.Service", "wwwroot", "css", "mobile.css");
        ...
        var attemptedPath = Path.Combine(attemptedBase, "EuphoriaInn.Service", "wwwroot", "css", "mobile.css");
        throw new FileNotFoundException(
            $"mobile.css not found. Searched upward from '{attemptedBase}'. " +
            $"Last attempted path: '{attemptedPath}'. " +
            "Ensure EuphoriaInn.Service/wwwroot/css/mobile.css exists in the repo.",
            attemptedPath);
    }
