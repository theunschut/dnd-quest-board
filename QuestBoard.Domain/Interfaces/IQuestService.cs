...
public interface IQuestService : IBaseService<Quest>
{
    ...
    Task UpdateQuestPropertiesAsync(int questId, string title, string description, int challengeRating, int dungeonMasterId, int totalPlayerCount, bool updateProposedDates = false, IList<DateTime>? proposedDates = null, CancellationToken token = default);
